...
@php
    ...
    $unreadOrder = Auth::user()->unread_orders()->withPivot('created_at')->orderBy('pivot_created_at', 'desc')->first();

    if(isset($unreadOrder)){

      $statusColor =[
            'NEW'=>'#2ecc71',

            'OVERDUE_CONFIRMED'=>'#fa2a00',
            'OVERDUE_PAYMENT'=>'#f39c12',
            'MISSED_PAYMENT'=>'#fa2a00',
      ];

      $ordersItemColor = $statusColor[ $unreadOrder->status ];
...
@foreach ($items as $item)
    ...
    <a {!! $linkAttributes !!} target="{{ $item->target }}" @if($item->route=='voyager.orders.index' && isset($ordersItemColor))style="color:{{$ordersItemColor}}"@else style="color:{{ (isset($item->color) && $item->color != '#000000' ? $item->color : '') }}"@endif>
