...
<article class="article">
  ...
  <div class="article__parallax">
    <div class="article__img-wrapper article__img-wrapper--outer">
      <img class="article__img"
          srcset="{{getThumbnail($item->image,600)}} 600w, {{getThumbnail($item->image,900)}} 900w, {{getThumbnail($item->image,1200)}} 1200w, {{getThumbnail($item->image,1500)}} 1500w, {{getThumbnail($item->image,1080)}} 1800w"
  ...
  <div class="container article__container">
    <div class="article__inner article__inner--content">
      <h1 class="article__title article__title--h1">{{$item->title}}</h1>
      <time class="article__published" datetime="{{$item->created_at}}">{{locale_date($item->created_at, '%d %s, %d')}}</time>
      <div class="article__about">
        {!! $content !!}
      </div>
      <ul class="likely">
        <li class="facebook" tabindex="0" role="link" aria-label="Share on Facebook">Поделиться</li>
        <li class="twitter" tabindex="0" role="link" aria-label="Tweet on Twitter">Твитнуть</li>
        <li class="telegram" tabindex="0" role="link" aria-label="Send on Telegram">Отправить</li>
      </ul>
    </div>
