<section class="schedule">
  <div class="schedule__container container">
    <h1 class="schedule__title">@lang('schedule.title')</h1>
                ...
                <li class="event eventlist__event">
                  ...
                  <div class="event__about">
                    <div class="event__text-wrapper">
                      <h3 class="event__title">{{$item->getTranslatedAttribute('title')}}</h3>
                      @foreach($item->badges as $badge)
                      <span class="event__badge">{{$badge->getTranslatedAttribute('name')}}</span>
                      @endforeach
                      @if(!empty($item->description))<div class="event__description">{!!$item->getTranslatedAttribute('description')!!}</div>@endif
                      @if(isset($item->persons))
                      <strong class="event__lector">@foreach($item->persons as $person){{$person->getTranslatedAttribute('name')}}<?php if($loop->remaining>0) echo ', ';?>@endforeach</strong>
                      @endif
                    </div>
                    @if(isset($item->partners))
                    <div class="event__meta-wrapper">
                      <div class="event__logo">
                      @foreach($item->partners as $partner)<img class="event__img" src="{{getOptimizedImage(get_file_url($partner->image))}}" width="120" alt="{{$partner->getTranslatedAttribute('name')}}">@endforeach
                      </div>
                    </div>
                    @endif
                  </div>
