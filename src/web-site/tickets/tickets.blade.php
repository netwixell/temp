<section class="tickets" id="tickets">
  <div class="tickets__container container">
    <h2 class="tickets__title">@lang('tickets.title')</h2>
    <ol class="steps tickets__steps">
      <li class="steps__step"><span>{!!__('tickets.step_1')!!}</span></li>
      ...
      <ul class="tickets__list swiper-wrapper">
        ...
        <form method="POST" action="/buying">
          ...
          <div class="ticket__title-wrapper">
            <h3 class="ticket__title">{{$ticket->getTranslatedAttribute('flow')}}</h3>
          ...
          @endif
            ...
            @foreach($options as $option)
                ...
                <li class="ticket__offer">{{$option->getTranslatedAttribute('name')}}</li>
                @endif
            @endforeach
                ...
                ?>
                  <li class="ticket__offer">
                    <label class="toggler ticket__toggler">
                      ...
                      <p class="toggler__toggle toggler__toggle--yes">{{$first_option->getTranslatedAttribute('name')}}</p>
                      <div class="toggler__slider"></div>
                      <p class="toggler__toggle toggler__toggle--no">{{$second_option->getTranslatedAttribute('name')}}</p>
                    </label>
                  </li>
                @endif
            @endforeach
          ...
          <button class="ticket__button js-ticket__button js-ticket__button--buying button button--red" type="submit">
            <span>@lang('tickets.buy_cash')</span><b class="ticket__price ticket__price--white"></b>
          </button>
          <?php
          ...
          <span>@lang('tickets.buy_installments')</span><b class="ticket__price ticket__price--solid" data-mask="{{__('orders.installment-price')}}"></b>
          </button>
          @endisset
        </form>
        </li>
