                    ...
                    <div class="panel-body">
                        ...
                        <div class="table-responsive">
                            <table id="dataTable" class="table table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $unreads = DB::table('userables')->select('userable_id')->where('user_id', Auth::id())->where('userable_type', 'callback')->pluck('userable_id');
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            <td>
                                                ...
                                                @elseif($row->type == 'select_multiple')
                                                    ...
                                                    @endif
                                                 {{-- Custom status field --}}
                                                 @elseif($row->type == 'select_dropdown' && property_exists($options, 'options') && $row->field == 'status')
                                                  @if('PROCESSED' == $data->status)
                                                  <div class="btn-group" data-toggle="tooltip"><button type="button" class="btn btn-default">Обработано</button></div>
                                                  @else
                                                  <a href="{{route('voyager.'.$dataType->slug.'.update', $data->getKey()) }}" class="update-link">
...
@section('javascript')
    ...
    <script>
        ...
        $('.update-link').click(function(e){
          ...
          updateFormAction=$(this).attr('href');

          return false;
        })
        $('select[name="status"]').on('change',function(){
          var action=updateFormAction || $(this).parent().attr('href');
          $update_form.prop('action',action);
          $update_form.find('[name="status"]').val(this.value);
          $update_form.submit();
          //alert('hi'+this.value+' , '+updateFormAction);

        });
        $update_form.submit(function(e){
          $.ajax({
            url: $update_form.attr('action')+'?ajax=true',
            type: 'POST',
            data: $update_form.serialize(),
            success: function(result) {
              if(result.item_id) $('tr[data-id='+result.item_id+']').removeClass('warning');
