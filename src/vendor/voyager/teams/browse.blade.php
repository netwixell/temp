                            ...
                            <table id="dataTable" class="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        @endforeach
                                        <th>Судейские баллы</th>
                                        <th class="actions">{{ __('voyager::generic.actions') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    $unreads = DB::table('userables')->select('userable_id')->where('user_id', Auth::id())->where('userable_type', 'team')->get();
                                    $unreadIds = $unreads->pluck('userable_id')->toArray();
                                    ?>
                                    @foreach($dataTypeContent as $data)
                                    <tr @if(in_array($data->id, $unreadIds))class="warning"@endif>
                                        @can('delete',app($dataType->model_name))
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                @elseif($row->type == 'text')
                                                    ...
                                                    <div style="width:150px">
                                                      ...
                                                      <b>{{ $data->name }}</b>
                                                      <p><a href="tel:{{$data->phone}}" style="white-space:nowrap">{{phone_format($data->phone)}}</a><br><span>{{$data->contact_name}}</span></p>
                                                    </div>
                                                    @else
                                                    @include('voyager::multilingual.input-hidden-bread-browse')
