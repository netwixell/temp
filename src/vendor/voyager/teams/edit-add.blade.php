          ...
          <div class="panel panel-bordered panel-transparent">
            ...
            <div class="panel-body">
                ...
                @php
                    ...
                    $exclude = ['event_id','contact_name','phone','email', 'status', 'badge', 'notice', 'photos', 'created_at', 'updated_at'];
                ...
                <div class="form-group">
                  <br>
                  @if(isset($dataTypeContent->photos))
                      <?php
                        $images = $dataTypeContent->photoArray;
                        $photos = $dataTypeContent->responsive;
                      ?>
                      @if(isset($photos) && count($photos) > 1 && isset($images))
                          @foreach(array_slice($photos, 1) as $photo)
                              ...
                              <img src="{{$photo->src }}" data-image="{{ $images[$loop->index] }}" data-id="{{ $dataTypeContent->id }}" style="max-width:200px; height:auto; clear:both; display:block; padding:2px; border:1px solid #ddd; margin-bottom:5px;">
                  ...
                  <label for="photos[]">Фотографии. Максимум 8 шт. Первое загруженное фото — основное, используемое для карточки команды.<br>
                    Общий вес фотографий не должен превышать 50 МБ. Загрузка и обработка может занять до 5 минут
                  </label>
                  <input type="file" name="photos[]" multiple="multiple" data-limit="{{ 8 - ( isset($images) ? count($images) : 0) }}" id="photos" accept="image/*">
                </div>
                <div class="form-group" >
                  <label for="notice">Заметка</label>
                  <a href="#" id="show_notice"><i class="voyager-eye"></i> Показать</a>
                  <div class="hidden" id="notice_elm">
                    <textarea class="form-control simplemde" name="notice" id="markdownnotice">{{$dataTypeContent->notice}}</textarea>
                  </div>
                </div>
            </div><!-- panel-body -->
          </div>{{--panel--}}
              ...
              <div class="panel-body">
                  <div class="form-group">
                    ...
                    <select class="form-control" name="status">
                        @foreach($dataTypeContent::$statuses as $status)
                        <option value="{{$status}}"@if(isset($dataTypeContent->status) && $dataTypeContent->status == $status) selected="selected"@endif>{{__('teams.'.$status)}}</option>
                        @endforeach
                    </select>
                  </div>
                  <div class="form-group">
    ...
    <script>
        $('document').ready(function () {
            ...
            $('#photos').change(function(e){
                ...
                var limit =  parseInt($input.data('limit'));
                if(files.length > limit){
                    alert("Вы можете выбрать до "+limit+" изображений.");
                    $input.val('');
                    return false;
                } else {
                    return true;
