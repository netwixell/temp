    ...
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <ol class="breadcrumb hidden-xs">
                <li class="active">
                    <a href="javascript:location.replace(document.referrer);"><i class="voyager-angle-left"></i> Назад</a>
                </li>
                ...
                @for($i = 1; $i <= count(Request::segments()); $i++)
                    ...
                    @if(Request::segment($i) != ltrim(route('voyager.dashboard', [], false), '/') && !is_numeric(Request::segment($i)))
                        ...
                        @if($i < count(Request::segments()) & $i > 0 && array_search('database',Request::segments())===false)
                            <li class="active"><a
                                        href="{{ $breadcrumb_url }}">{{ ucfirst(str_replace('-', ' ', str_replace('_', ' ', Request::segment($i)))) }}</a>
                            </li>
                        @else
                            <li>{{ ucfirst(str_replace('-', ' ', str_replace('_', ' ', Request::segment($i)))) }}</li>
                        @endif
